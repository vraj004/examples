...
PROGRAM CELLMLSPLITREACDIFFLINEARCONVERGENCEEXAMPLE
  ...
  INTEGER(CMISSIntg) :: NUMBER_GLOBAL_X_ELEMENTS,CONDITION,NUMBER_GLOBAL_Y_ELEMENTS,RYRNODE
  ...
  CHARACTER(KIND=C_CHAR,LEN=*), PARAMETER :: outputFilename = &
    & outputDirectory//"/reacdiff.xml"
  CHARACTER(KIND=C_CHAR,LEN=*), PARAMETER :: basename = "reacdiff"
  ...
  !ALLOCATE(RYRNODES(NODESXY)) used to set an entire mid plane as sources.
  !setting only one node as source in the center of the box.
  RYRNODE = INCNODE*((NUMBER_GLOBAL_Y_ELEMENTS+1)* &
    & (1+NUMBER_GLOBAL_Z_ELEMENTS/2_CMISSIntg))- &
    & INCNODE*(NUMBER_GLOBAL_Y_ELEMENTS/2_CMISSIntg)-((INCNODE-1)/2_CMISSIntg)
  WRITE(*,*) RYRNODE
  ...
  !Set up to have sources on entire mid-plane
  !DO node=1,NODESXY
  ...
  !  ENDIF
  !ENDDO
  !Setting up source at central node in mesh
  NODE_NUMBER=RYRNODE
  CALL CMISSDecomposition_NodeDomainGet(Decomposition,NODE_NUMBER,1,NodeDomain,Err)
  IF(NodeDomain==ComputationalNodeNumber) THEN
    CALL CMISSField_ParameterSetUpdateNode(SourceField,CMISS_FIELD_U_VARIABLE_TYPE, &
