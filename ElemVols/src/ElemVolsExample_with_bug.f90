...
PROGRAM ELEMVOLSEXAMPLE
  ...
  !Start the creation of a generated mesh in the region
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !BUG WHEN GENERATING REGULAR MESH WITH SIMPLEX BASES - THIS EXAMPLE SHOWS THAT WHEN ASKING FOR TWO ELEMENTS IN X AND 1 IN Y AND Z
  ! THE CODE GENERATES 12 ELEMENTS (AS SEEN IN CMGUI) BUT STILL ONLY RECOGNIZES TWO ELEMENTS THAT SUM TO THE TOTAL OF 1000 CUBIC UNIT VOLUME.
  ...
  DO ne=1,NUMBER_GLOBAL_ELEMENTS
    CALL CMISSField_GeometricParametersElementVolumeGet(GeometricField,ne,elem_volume,Err)
    WRITE(*,*) "Elem number", ne !print out element
    ...
    CALL CMISSField_ParameterSetUpdateElement(ElemVolField,CMISS_FIELD_U_VARIABLE_TYPE,CMISS_FIELD_VALUES_SET_TYPE, &
      & ne,GeometricMeshComponent,elem_volume,Err)
  ENDDO
  

  !Find surrounding elements of a particular node.
  NODE_NUMBER=1_CMISSIntg
